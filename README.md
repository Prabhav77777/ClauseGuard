...
- **Cross-Clause Inconsistency Detection**: Implemented `backend/prompts/inconsistency.py` and `GET /api/documents/{session_id}/inconsistencies` leveraging `InconsistencyResult` schema. Detected conflicts are lazily computed once and cached on `DocumentSession.inconsistencies` for $O(1)$ repeat access.
- **Narrowed Content-Cache Lock Scope**: Refactored `upload_document` in `backend/api/documents.py` to hold `_content_cache_lock` only during dictionary reads/writes, running `process_document(...)` unlocked. Concurrent uploads of different documents now execute in parallel without lock serialization.
- **Read Endpoint Rate Limiting**: Applied `@limiter.limit("60/minute")` across all read endpoints (`GET /clauses`, `GET /clauses/{id}`, `GET /brief`, `GET /inconsistencies`) in `backend/api/documents.py` to protect against session ID enumeration attacks while allowing normal UI navigation.
- **Structural Anti-Resolution Enforcement**: Documented that the "never resolves controlling clause" requirement is enforced structurally by Pydantic schema design (`InconsistencyResult` contains only `clause_ids` and `description` without winning clause fields) and system prompt instructions.
...
- **Lifespan State Binding in API Integration Tests**: Identified that instantiated `TestClient(app)` calls without context manager startup do not invoke FastAPI `lifespan` handlers, leaving `app.state.sessions` unbound to `main.sessions`. Updated `test_session_expiry_integration` in `tests/test_performance.py` to explicitly bind state references.
...
| **Highlighting important clauses, obligations, risks, or inconsistencies** | Fixed taxonomy clause map auto-categorizing compensation, termination, non-compete, liability, etc., plus cross-clause inconsistency detection. | `backend/prompts/categorization.py`, `backend/prompts/inconsistency.py` |
| **Detecting internal clause contradictions** | Cross-clause inconsistency detection finding contradictory provisions (e.g. 30-day vs 60-day notice) without declaring a winning clause. | `backend/prompts/inconsistency.py` |
